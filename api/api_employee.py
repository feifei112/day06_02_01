...
class ApiEmployee:
    ...
    def api_put_employee(self):
        ...
        # 2. 调用put方法 ->返回响应对象
        return requests.put(url=self.url_put.format(api.user_id), json=data,
                            headers=api.headers)
    ...
    def api_get_employee(self):
        ...
        return requests.get(url=self.url_get.format(api.user_id),
                            headers=api.headers)
    ...
    def api_delete_employee(self):
        ...
        # 1. 调用delete方法 ->返回响应对象
        return requests.delete(url=self.url_delete.format(api.user_id),
                               headers=api.headers)
